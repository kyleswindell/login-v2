...
This memory governs bounded work against the current Carbon CSS implementation only.

References to `resources/css/components/*`, `resources/css/tokens/*`, `resources/css/type/*`, and `resources/css/base/*` identify transitional current implementation paths. They do not define target repository topology or create permanent ownership layers.

Do not create new architectural categories or expand these paths as target structure. Existing files may receive only bounded maintenance or migration work explicitly authorized by the governing issue. Target UI source colocation is defined by `docs/03-architecture/repository-architecture.md`.

When artifact-owned presentation bundles replace these paths, update or supersede this memory in the same accepted migration.
