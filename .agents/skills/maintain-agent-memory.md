...
- If a memory item has become canonical truth, do NOT rewrite `docs/` here; instead flag the required canonical promotion clearly
- If a memory item has become a durable repo rule, workflow behavior, or reusable starter-pack improvement, do NOT leave it in memory; flag the required promotion into `AGENTS.md`, the relevant skill file, or `.agents/baselines/`
