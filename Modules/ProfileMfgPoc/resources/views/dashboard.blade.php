{{-- ==========================================================================
    ...
    Purpose: Daily and weekly shipping coordination home for the static POC.
    ========================================================================== --}}
...
<x-layouts.app
    ...
    :page-subtitle="'Daily and weekly customer demand, packaging context, and finished-goods readiness through '.$schedule_end_label.'.'"
...
>
    <x-slot:pageActions>
        <x-ui.button kind="secondary" disabled>Print schedule — Preview</x-ui.button>
        <x-ui.button kind="primary" disabled>Close / short orders — Preview</x-ui.button>
    </x-slot:pageActions>
    ...
    <div class="space-y-6">
        ...
        <x-patterns.dashboard-grid columns="4" aria-label="Shipping workload summary">
            @include('profile-mfg-poc::partials.summary-tile', [
                ...
                'status' => $dashboard['past_due_demand'] > 0 ? 'Requires recovery plan' : null,
                'statusTone' => 'danger',
            ])
            @include('profile-mfg-poc::partials.summary-tile', [
                'label' => 'Ship today',
        ...
        @include('profile-mfg-poc::partials.shipping-schedule-table', [
            ...
            'description' => 'The current and next work week remain the primary coordination window. Daily cells link to the orders that make up each remaining quantity; forward columns preserve the longer planning horizon.',
        ])
        ...
        @include('profile-mfg-poc::partials.inventory-table', [
            ...
            'description' => 'Parts with near-term demand where system and serialized stock are short, conflicting, or unavailable.',
        ])
        ...
        <x-ui.contained-list
            ...
            variant="on-page"
            size="md"
            :items="collect($dashboard['shipping_notes'])->map(fn ($customer) => [
                'title' => $customer['name'],
                'description' => collect([
                    $customer['shipping_instructions'] ?: 'No shipping instructions were supplied.',
                    filled(collect($customer['shipping_address'] ?? [])->filter()->implode(', '))
                        ? 'Ship to: '.collect($customer['shipping_address'])->filter()->implode(', ')
                        : null,
                ])->filter()->implode(' · '),
