...
class UiReferenceComponentCatalog
{
    ...
    private function components(): array
    {
        return [
            ...
            $this->component('combo-box', 'Combo box', 'Inputs', 'Queued Gap', 'Typed known-option selection and optional custom-value entry require a dedicated component API before implementation.', ['queued', 'typeahead', 'filtering', 'clear control', 'custom value gate', 'keyboard contract'], ['Use this page to confirm Combo box is a separate owner from Dropdown and Multiselect.', 'Do not build typed selection behavior inside Dropdown.']),
            ...
            $this->component('date-picker', 'Date picker', 'Inputs', 'Implemented Pending Review', 'Simple date input, single calendar, range calendar, and skeleton proofs for date entry.', ['date', 'simple date input', 'single calendar', 'date range calendar', 'small', 'medium', 'large', 'fluid', 'error', 'warning', 'disabled', 'read-only', 'loading'], ['Use date picker for date entry and range selection when users need calendar context. Use Forms Patterns for submission and workflow validation.']),
    ...
    private function priorityFor(string $slug): string
    {
        return match ($slug) {
            ...
            'accordion', 'code-snippet', 'combo-box', 'contained-list', 'content-switcher', 'dropdown', 'file-uploader', 'list', 'menu', 'menu-buttons', 'multiselect', 'number-input', 'progress-bar', 'progress-indicator', 'tabs', 'tile', 'toggletip' => 'B',
            default => 'C',
        };
    }
    ...
    private function anatomyFor(string $slug): array
    {
        return match ($slug) {
            ...
            'text-input', 'select', 'dropdown', 'combo-box', 'number-input', 'date-picker', 'file-uploader', 'search', 'slider', 'multiselect' => ['label', 'control', 'helper text', 'validation message', 'optional status icon'],
