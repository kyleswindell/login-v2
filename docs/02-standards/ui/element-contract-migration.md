...
| ----------------------------------------- | ------------------------------------------------- | ------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------- |
| `docs/02-standards/ui/elements/tokens.md` | Token Governance Element standard                 | Confirmed by current architecture; verify in contracts | Existing standards file for token governance; not a token source file.                                      |
| `resources/css/tokens/themes/**`          | Themes / Color                                    | Confirmed by current architecture; verify in contracts | Themes owns theme value sets and contexts; Color owns semantic color roles those values satisfy.            |
| `resources/css/tokens/semantic/**`        | Color                                             | Confirmed by current architecture; verify in contracts | Semantic token aliases map palette/theme values into app color roles.                                       |
| `resources/css/tokens/palette/**`         | Color / Themes                                    | Confirmed by current architecture; verify in contracts | Palette files provide primitive source values; usage must remain role-driven through Color and Themes.      |
| `resources/css/tokens/spacing.css`        | Spacing                                           | Confirmed by current architecture; verify in contracts | Spacing owns the scale and relationship model.                                                              |
| `resources/css/tokens/motion.css`         | Motion                                            | Confirmed by current architecture; verify in contracts | Motion owns duration/easing roles and reduced-motion behavior.                                              |
| `resources/css/tokens/type/index.css`     | Typography                                        | Confirmed by current architecture; verify in contracts | Typography owns font, scale, role, and weight tokens.                                                       |
...
| `resources/css/tokens/components/**`      | Component-owned tokens, dependent on Color/Themes | Confirmed by current architecture; verify in contracts | Component token files depend on Color and Themes but remain private to approved Component token families.   |
...
- Decide whether `resources/css/tokens/shadow.css` and `resources/css/tokens/z-index.css` belong to Elements, Patterns, UI Shell, shared infrastructure, or a documented split.
