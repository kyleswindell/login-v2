...
| ------------------- | --------------------- | ---------------------- | --------------------- | ---------------------- | -------------- | ---------------- | ----- |
| `$button-secondary` | Secondary button container | Carbon Button style / theme token | `--ui-action-secondary-bg` via `<x-ui.button semantic="secondary">` | App action palette | Same role / app value | App action palette is intentionally tuned, but the role is still secondary action. | Button Component |
...
| `$content-switcher-*` | Content Switcher Component | Content switcher component tokens when installed | Component-owned; do not express as generic tabs/buttons unless the Component standard maps it. |
