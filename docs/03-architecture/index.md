# Architecture Index

This index provides canonical navigation and scope for this branch.

## Purpose

Canonical hub for system structure, boundaries, ownership, and high-level design.

## Scope

Structure and architecture ownership only. Behavior, flow steps, schema details, planning, support notes, and operations procedures are owned by other branches.

## Core Architecture Docs

- [System Overview](system-overview.md)
- [App 2.0 Blueprint](app-2-0-blueprint.md)
- [App 2.0 Blueprint Architecture Baseline](app-2-0-blueprint-architecture-baseline.md)
- [Phase 1 Foundation Architecture Direction](phase-1-foundation-architecture-direction.md)
- [Stack Overview](stack-overview.md)
- [Platform Boundary](platform-boundary.md)
- [Tenancy](tenancy.md)
- [Auth Architecture](auth.md)
- [Phase 4 UI Ownership Map](phase-4-ui-ownership-map.md)

## Subsystems

- [Core Platform Layer Model](subsystems/core-platform-layer-model.md)
- [Application Structure](subsystems/application-structure.md)
- [Current Repo Structure](subsystems/current-repo-structure.md)
- [Final Stack And UI Boundary](subsystems/final-stack-and-ui-boundary.md)
- [Phase 2 Stack And UI Architecture Decisions](subsystems/phase-2-stack-and-ui-architecture-decisions.md)
- [Dashboard Subsystem](subsystems/dashboard.md)
- [Inter-Tenant Messaging Subsystem](subsystems/inter-tenant-messaging.md)

## Related

- [00-start-here](../00-start-here.md)
- [Standards Index](../02-standards/index.md)
- [Database Index](../06-database/index.md)
- [Tenant Safety Standards](../02-standards/security/Tenant%20Safety%20Standards.md)
- [Phase 2 Stack And UI System Notes](../09-reference/architecture/phase-2-stack-and-ui-system-notes.md)
