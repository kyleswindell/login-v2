# Features Index

This index provides canonical navigation and scope for this branch.

## Purpose

Canonical hub for feature behavior, contracts, and feature-scoped dependencies.

## Scope

Feature behavior only. Architecture structure, operational runbooks, and implementation sequencing are not owned here.

## Implemented Foundation Features

- [Authentication](auth/authentication.md)
- [Event And Error Logging](logging/event-and-error-logging.md)
- [Platform Users And RBAC](users/platform-users-and-rbac.md)
- [Platform Notifications And Settings](notifications/platform-notifications-and-settings.md)
- [Realtime Notifications And Broadcasting](notifications/realtime-notifications-and-broadcasting.md)
- [Platform Workspace And Documentation Vault](workspace/platform-workspace-and-documentation-vault.md)
- [Dashboard](dashboard/dashboard.md)
- [Account Management And Settings](account/account-management-and-settings.md)

## Drafted Contracts

- [Calendar And CalendarEntry Contract](calendar/calendar-and-calendar-entry-contract.md)
- [Inter-Tenant Messaging Contract](tenants/inter-tenant-messaging-contract.md)

## Domain Groups

- `auth/`
- `users/`
- `tenants/`
- `logging/`
- `notifications/`
- `dashboard/`
- `workspace/`
- `account/`
- `calendar/`

## Related

- [00-start-here](../00-start-here.md)
- [Standards Index](../02-standards/index.md)
- [Architecture Index](../03-architecture/index.md)
- [Database Index](../06-database/index.md)
- [Flows Index](../05-flows/index.md)
- [Planning Index](../07-planning/index.md)

## Flow References

- Implemented: [Login Authentication Flow](../05-flows/login-authentication-flow.md)
- Planned: [Account Password Change Flow](../05-flows/account-password-change-flow.md)
- Planned: [Dashboard Customization Flow](../05-flows/dashboard-customization-flow.md)
- Planned: [Notification Read And Dismiss Flow](../05-flows/notification-read-and-dismiss-flow.md)
- Planned: [Customer Access And OAuth Flow](../05-flows/customer-access-and-oauth-flow.md)
- Planned: [Inter-Tenant Messaging Flow](../05-flows/inter-tenant-messaging-flow.md)
- Planned: [Tenant Provisioning Flow](../05-flows/tenant-provisioning-flow.md)
