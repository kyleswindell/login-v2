# Flows Index

This index provides canonical navigation and scope for this branch.

## Purpose

Canonical hub for execution paths and ordered system/user flows.

## Scope

Execution sequence documentation only. This branch does not own feature contracts, architecture boundaries, schema details, or operational runbooks.

## Implemented Flows

- [Login Authentication Flow](login-authentication-flow.md)

## Planned Flows

- [Account Password Change Flow](account-password-change-flow.md)
- [Brochure Astro Delivery Flow](brochure-astro-delivery-flow.md)
- [Brochure Integrated Delivery Flow](brochure-integrated-delivery-flow.md)
- [Dashboard Customization Flow](dashboard-customization-flow.md)
- [Notification Read And Dismiss Flow](notification-read-and-dismiss-flow.md)
- [Tenant Provisioning Flow](tenant-provisioning-flow.md)
- [Customer Access And OAuth Flow](customer-access-and-oauth-flow.md)
- [Inter-Tenant Messaging Flow](inter-tenant-messaging-flow.md)

## Related

- [00-start-here](../00-start-here.md)
- [Features Index](../04-features/index.md)
- [Planning Index](../07-planning/index.md)
