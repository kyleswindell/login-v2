# Database Index

This index provides canonical navigation and scope for this branch.

## Purpose

Canonical hub for schema, tables, and data contracts.

## Scope

Schema and data contracts only. Feature behavior, architecture rationale, operational procedures, and phase sequencing are owned in other branches.

## Feature Data Contracts

- [Auth And RBAC](feature-contracts/auth-and-rbac.md)
- [Brochure Sites Data Contract](feature-contracts/brochure-sites.md)
- [Notifications And Settings](feature-contracts/notifications-and-settings.md)
- [Event And Error Logging](feature-contracts/logging.md)
- [Dashboard Layout](feature-contracts/dashboard-layout.md)
- [Calendar Contracts](feature-contracts/calendar.md)
- [Inter-Tenant Messaging](feature-contracts/inter-tenant-messaging.md)

## Schema

- [Schema](schema.md)
- [Tables Index](tables/index.md)

## Canonical Tables

- [platform_audit_logs](tables/platform_audit_logs.md)
- [central_error_logs](tables/central_error_logs.md)
- [notifications](tables/notifications.md)
- [settings](tables/settings.md)

## Schema Contracts Extracted From Batches

- [Phase 1 Batch 1 Foundation Schema Contract](phase-1-batch-1-foundation-schema-contract.md)
- [Phase 2 Batch 11 Dashboard Schema Contract](phase-2-batch-11-dashboard-schema-contract.md)

## Related

- [00-start-here](../00-start-here.md)
- [Features Index](../04-features/index.md)
- [Database Migration Standards](../02-standards/database/Database%20Migration%20Standards.md)
- [Phase 1 - Foundation Data Direction](../07-planning/phases/phase-1/Phase%201%20-%20Foundation%20Data%20Direction.md)
