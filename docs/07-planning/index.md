# Planning Index

This index provides canonical navigation and scope for this branch.

## Purpose

Canonical hub for roadmap, phase sequencing, and delivery batches.

## Scope

Planning sequence and delivery intent only. This branch does not own final architecture structure, feature behavior contracts, schema contracts, or runbook operations.

## Core Planning

- [Roadmap](roadmap.md)
- [Dependency Map](dependency-map.md)
- [Phases](phases/index.md)
- [Batches](batches/index.md)
- [App 2.0 Blueprint Planning](app-2-0-blueprint-planning.md)
- [App 2.0 Blueprint Initial Build Order](app-2-0-blueprint-initial-build-order.md)

## Brochure Planning

- [Phase 3 - Brochure Subsystem Hybrid Delivery Planning](phases/phase-3/Phase%203%20-%20Brochure%20Subsystem%20Hybrid%20Delivery%20Planning.md)

## Related Reference Notes

- [Modern Commenting Standards Research](../09-reference/documentation/Modern%20Commenting%20Standards%20Research.md)
- [App 2.0 Logging DB Columns Research](../09-reference/architecture/app-2-0-logging-db-columns-research.md)

## Related

- [00-start-here](../00-start-here.md)
- [Features Index](../04-features/index.md)
- [Flows Index](../05-flows/index.md)
