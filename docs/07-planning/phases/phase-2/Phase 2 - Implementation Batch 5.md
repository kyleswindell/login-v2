...
This document defines the canonical scope and intent for Phase 2 - Implementation Batch 5.
...
* preserve documentation-vault custom behavior unless explicitly re-scoped
...
| Platform users | Filament candidate, service-backed | `/platform/administration/users` redirects to app-owned `/platform/users`; current `/platform/users/*` remains the primary surface while direct `/console/platform-users` proof access is optional | `manage-platform-users` | current Blade `/platform/users/*` and direct `/console/platform-users` proof path | Filament parity hardening implemented; shell navigation now points to the target route. |
