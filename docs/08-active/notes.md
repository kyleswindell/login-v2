...
- P2-F-CQ-153 is Implemented Pending Review after worklog 2-F-0053. Checkbox now uses a dedicated proof matrix for independent choice, multi-select group, states, group states, nesting, overflow, and alignment, with `initCheckboxes` owning parent/child mixed-state synchronization.
