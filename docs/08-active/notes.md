...
- P2-F-CQ-079 is Implemented Pending Review. Button now uses the broad matrix-heavy proof layout with corrected size classes, icon-only state coverage, loading support, and danger icon-only prohibition.
- P2-F-CQ-093 is Implemented Pending Review. Menu buttons now prove the installed `x-ui.menu-button`, `x-ui.combo-button`, and `x-ui.overflow-menu` APIs with closed interactive base examples plus explicit state/anatomy proof.
- P2-F-CQ-136 through P2-F-CQ-163 are Implemented Pending Review. The remaining Component API proof/recovery pages now use installed APIs or explicit disposition/gap ownership instead of generic placeholders.
- Starter items P2-F-CQ-002 through P2-F-CQ-006 remain blocked behind manual review of component recovery, now represented by P2-F-CQ-077, P2-F-CQ-079, P2-F-CQ-093, and P2-F-CQ-136 through P2-F-CQ-163.
...
- P2-F-CQ-074, P2-F-CQ-075, P2-F-CQ-076, and P2-F-CQ-078 are Implemented Pending Review. Worklog 2-F-0025 starts component-by-component recovery with Breadcrumb, Tabs, Code snippet, and a generic fallback ban.
- P2-F-CQ-079 is unblocked by the Menu correction. Worklog 2-F-0026 adds the flexible Live examples layout and Button matrix proof, but Button still requires final correction/review against its expanded variant/size/group/icon/content requirements.
...
- P2-F-CQ-074 received a focused Breadcrumb correction after manual review: default trails now stop at the previous page with a trailing separator, overflow keeps the first two and final two page links visible, current-page-listed overflow keeps five page items visible, and overflow menus render open on the reference page for review.
...
- Manual review should focus next on the full component recovery set: `/platform/ui-reference/components/breadcrumb`, `/platform/ui-reference/components/tabs`, `/platform/ui-reference/components/code-snippet`, `/platform/ui-reference/components/menu`, `/platform/ui-reference/components/button`, `/platform/ui-reference/components/menu-buttons`, and the remaining `P2-F-CQ-136` through `P2-F-CQ-163` routes starting with `/platform/ui-reference/components/link`.
