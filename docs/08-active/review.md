...
- Worklog `2-A-0013` retunes the unread trigger glow and adds an in-place header `Mark all as read` runtime path; those notification updates are now deployed to staging and pending another manual review pass.
...
## Required Fixes
- Manually review the worklog `2-A-0013` unread trigger glow retune so the bell still reads as unread without feeling overly luminous or decorative in the shared header shell.
- Manually review the worklog `2-A-0013` in-place header `Mark all as read` behavior so unread counts, dropdown rows, and trigger state update without a full page refresh.
- Re-run manual review on the full notification dropdown surface on staging, including the unread trigger treatment, the approved unread-row treatment, the approved realtime generated-notification path, and the in-place `Mark all as read` behavior.
