...
- Worklog `2-A-0011` now carries the current notification visual follow-up pass and still needs deployment plus manual re-review on staging before Batch A can move forward.
...
- Re-run manual visual review on the full notification dropdown surface after deployment, including the passed `Mark all as read` path, the danger trigger treatment, and the stronger unread-row treatment.
