{{-- ==========================================================================
    File: resources/views/components/ui/multi-select/index.blade.php
    Purpose: MultiSelect/ListBox form control component.

    Notes:
    - Emits the installed .ui-multi-select and .ui-list-box selector contract.
    - Renders a button trigger, selected-count affordance, option menu, and
      hidden inputs for submitted selected values.
    - Supports selected items, select-all item, helper text, invalid, warning,
      disabled, read-only, inline, light, size, direction, and decorator content.
    - Open/close, keyboard navigation, selection, selected-count, sorting, and
      hidden input sync are handled by installed MultiSelect JavaScript.
    - Uses the unified x-ui.icon component for icons.
    - MultiSelect styles are handled by resources/css/components/multi-select.css
      and resources/css/components/list-box.css.
    ========================================================================== --}}
...
@php
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | `titleText` is the field label. `label` is the trigger/display label.
    | `slug` is retained as a legacy alias for `decorator`.
    |
    */

    $resolvedId = $id ?? 'ui-multi-select-'.Str::uuid();
    $menuId = $resolvedId.'-menu';
    $labelId = $resolvedId.'-label';
    $fieldLabelId = $resolvedId.'-field-label';
    ...
    */
    ...
    $resolvedTriggerLabel = $label ?? ($selectedCount > 0 ? $selectedCount.' selected' : 'Select options');
...
<div
    {{ $wrapperAttributes->class($wrapperClasses)->merge([
        'data-ui-component' => 'multi-select',
...
>
    ...
    @if (filled($titleText))
        <label id="{{ $labelId }}" for="{{ $triggerId }}" @class($labelClasses)>
    ...
    <div
        ...
        data-ui-multi-select-clear-announcement-text="{{ $clearAnnouncement }}"
    ...
    >
        ...
        <div @class($fieldWrapperClasses) data-ui-multi-select-field-wrapper>
            {{-- ------------------------------------------------------------------
            ...
            <button
                ...
                @unless ($selectedCount > 0) hidden @endunless
            >
            ...
            <button
                id="{{ $triggerId }}"
                type="button"
                class="ui-list-box__field"
                role="combobox"
                aria-haspopup="listbox"
                aria-expanded="{{ $isOpen ? 'true' : 'false' }}"
                aria-controls="{{ $menuId }}"
                @if (filled($titleText)) aria-labelledby="{{ $fieldLabelId }} {{ $labelId }}" @else aria-labelledby="{{ $fieldLabelId }}" @endif
            ...
            >
                <span id="{{ $fieldLabelId }}" class="ui-list-box__label">
                    @if ($resolvedTriggerLabel instanceof HtmlString)
    ...
    @if ($showInvalid)
        <div
            ...
            data-ui-multi-select-validation
        >
            {{ $invalidText }}
        </div>
    @elseif ($showWarning)
