<x-layouts.app title="System Update Settings">
    ...
    <section class="flex flex-1 flex-col gap-6">
        ...
        <form method="POST" action="{{ route('platform.settings.general.system-update.update') }}" class="rounded-3xl border border-slate-800 bg-slate-900/70 p-8 shadow-2xl shadow-black/30">
            @csrf
            <div class="grid gap-6 md:grid-cols-2">
                <label class="block">
                    <span class="text-sm font-semibold text-slate-200">Update Channel</span>
                    <select name="update_channel" class="mt-2 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-slate-100" required>
                ...
                <label class="block">
                    ...
                    <input type="text" name="maintenance_window" value="{{ old('maintenance_window', $maintenanceWindow) }}" class="mt-2 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-slate-100">
                </label>

                <label class="flex items-center gap-3 rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-sm text-slate-200 md:col-span-2">
            ...
            <div class="mt-8 border-t border-slate-800 pt-6">
                <button type="submit" class="rounded-2xl bg-sky-500/15 px-6 py-3 text-sm font-semibold text-sky-200 ring-1 ring-sky-500/30">Save Update Policy</button>
            </div>
        </form>
    </section>
</x-layouts.app>
