<x-layouts.app title="Platform User Settings">
    ...
    <section class="flex flex-1 flex-col gap-6">
        <div class="rounded-3xl border border-slate-800 bg-slate-900/70 p-8 shadow-2xl shadow-black/30">
            <p class="text-sm font-medium uppercase tracking-[0.3em] text-sky-300">Settings — Platform Users</p>
            <h1 class="mt-3 text-3xl font-semibold text-white">User Defaults</h1>
            <p class="mt-2 text-slate-400">Configure default role assignment and active state for newly created platform users.</p>
        </div>
        ...
        <form method="POST" action="{{ route('platform.settings.users.update') }}" class="rounded-3xl border border-slate-800 bg-slate-900/70 p-8 shadow-2xl shadow-black/30">
            @csrf
