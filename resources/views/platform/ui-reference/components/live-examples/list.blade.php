...
<div class="space-y-6" data-component-live-layout="list-matrix">
    @foreach ($examples as $example)
        <section class="rounded-lg border p-4" style="border-color: var(--ui-border-subtle-01); background-color: var(--ui-layer-01);" data-list-live-section="{{ $example['id'] }}">
            ...
            <div class="mt-4 rounded-md border p-4" style="border-color: var(--ui-border-subtle-01); background-color: var(--ui-layer-02);">
                @include('platform.ui-reference.components.live-examples.partials.list-proof', ['sample' => $example['sample'] ?? []])
            </div>

            @if (! empty($example['variants']))
                <div class="mt-5 grid gap-3 md:grid-cols-2">
                    @foreach ($example['variants'] as $variant)
                        <article class="min-w-0 rounded-md border p-3" style="border-color: var(--ui-border-subtle-01); background-color: var(--ui-layer-02);" data-list-live-variant="{{ Str::slug($variant['label']) }}">
                            ...
                            @include('platform.ui-reference.components.live-examples.partials.list-proof', ['sample' => $variant['sample'] ?? []])
