...
<div class="space-y-6" data-component-live-layout="tabs-matrix">
    @foreach ($examples as $example)
        <section class="rounded-lg border p-4" style="border-color: var(--ui-border-subtle-01); background-color: var(--ui-layer-01);" data-tabs-live-section="{{ $example['id'] }}">
            ...
            <div class="mt-4">
                @include('platform.ui-reference.components.live-examples.partials.tabs-proof', ['sample' => $example['sample'] ?? []])
            </div>

            @if (! empty($example['variants']))
                <div class="mt-5 grid gap-3 xl:grid-cols-2">
                    @foreach ($example['variants'] as $variant)
                        <article class="min-w-0 rounded-md border p-3" style="border-color: var(--ui-border-subtle-01); background-color: var(--ui-layer-02);" data-tabs-live-variant="{{ Str::slug($variant['label']) }}">
                            ...
                            @include('platform.ui-reference.components.live-examples.partials.tabs-proof', ['sample' => $variant['sample'] ?? []])
