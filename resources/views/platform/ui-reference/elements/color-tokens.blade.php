<x-layouts.app :title="'UI Reference - Color Token Palette'">
    ...
    <section class="flex min-w-0 flex-1 flex-col gap-6" data-ui-reference-foundation-element="color" data-ui-reference-color-token-palette>
        <div>
            ...
            <p class="ui-page-header-copy">Carbon-depth token role coverage for Login App 2.0, separated from the Color Overview so the token matrix can stay reviewable and complete.</p>
        </div>

        <section class="ui-card" data-color-token-section="inventory-map">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    <h2 class="ui-card-title">Carbon-To-Login Token Inventory</h2>
                    <p class="ui-card-copy mt-2">Every Carbon color-token family is mapped to a Login App disposition. Carbon values inform contrast, hierarchy, layer depth, state behavior, and semantic clarity; app tokens remain owned by Login App.</p>
                </div>
            ...
            <div class="mt-5 overflow-x-auto rounded-lg border border-slate-800 bg-slate-950/60">
                <table class="w-full min-w-[1040px] table-fixed divide-y divide-slate-800">
                    <colgroup>
                        <col class="w-[12rem]">
                        <col class="w-[13rem]">
                        <col class="w-[13rem]">
                    ...
                    <thead class="bg-slate-900">
                        <tr class="text-left text-xs uppercase tracking-[0.18em] text-slate-500">
                            ...
                            <th class="px-4 py-3">Carbon Family</th>
                    ...
                    <tbody class="divide-y divide-slate-800 text-sm text-slate-300">
                        @foreach ($tokenInventory as $item)
                            <tr data-color-token-inventory-family="{{ Str::slug($item['family']) }}">
                                <td class="px-4 py-3 font-semibold text-white">{{ $item['family'] }}</td>
                                <td class="px-4 py-3">{{ $item['carbon_family'] }}</td>
                                <td class="px-4 py-3">
                                    <span class="inline-flex items-center whitespace-nowrap rounded-full border border-slate-700 px-2.5 py-1 text-xs font-semibold text-slate-200">{{ $item['disposition'] }}</span>
                                </td>
                    ...
                    <table class="w-full min-w-[1120px] table-fixed divide-y divide-slate-800">
                        ...
                        <thead class="bg-slate-900">
                            <tr class="text-left text-xs uppercase tracking-[0.18em] text-slate-500">
                                ...
                                <th class="px-4 py-3">Carbon Comparison</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-800 text-sm text-slate-300">
                            @foreach ($family['rows'] as $row)
                                <tr data-color-token-variable="{{ $row['variable'] }}">
                                    <td class="px-4 py-3 font-semibold text-white">{{ $row['family'] }}</td>
                                    <td class="px-4 py-3">{{ $row['role'] }}</td>
                                    <td class="px-4 py-3 font-mono text-xs text-slate-200">{{ $row['variable'] }}</td>
                                    <td class="px-4 py-3 font-mono text-xs">{{ $row['light'] }}</td>
                                    <td class="px-4 py-3 font-mono text-xs">{{ $row['dark'] }}</td>
                                    <td class="px-4 py-3">
                                        <div class="flex gap-2">
                                            <span class="h-8 w-8 rounded border border-slate-700" style="background: {{ $row['light'] }};" aria-label="Light value swatch"></span>
                                            <span class="h-8 w-8 rounded border border-slate-700" style="background: {{ $row['dark'] }};" aria-label="Dark value swatch"></span>
                                        </div>
                                    </td>
