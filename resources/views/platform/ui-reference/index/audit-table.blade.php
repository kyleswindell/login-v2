            ...
            <form method="GET" action="{{ route('platform.ui-reference.index') }}" class="mt-4 hidden ui-reference-example-surface p-5" data-filter-panel>
                ...
                <div class="grid gap-4 md:grid-cols-3">
                    <label>
                        ...
                        <select name="audit_severity" class="ui-select mt-2">
                    ...
                    <label>
                        <span class="text-sm font-semibold ui-reference-text-strong">Result</span>
                        <select name="audit_result" class="ui-select mt-2">
                            <option value="">Any</option>
                    ...
                    <label>
                        ...
                        <input type="text" name="audit_search" value="{{ $auditFilters['search'] }}" placeholder="Event, actor, route" class="ui-input mt-2" />
                    </label>
                </div>
                ...
                <table class="min-w-[920px] w-full ui-reference-table-body">
                    ...
                    <tbody class="ui-reference-table-body text-sm ui-reference-text-strong">
                        @forelse ($auditSamples as $sample)
                            <tr class="ui-reference-table-row cursor-pointer transition" data-audit-log-row data-audit-log-url="{{ route('platform.ui-reference.audit-samples.show', $sample['sample_key']) }}">
                                <td class="px-5 py-3 ui-reference-text-muted">{{ $sample['occurred_at_label'] }}</td>
                                <td class="px-5 py-3 font-semibold ui-reference-text-strong">{{ $sample['event_type'] }}</td>
                                <td class="px-5 py-3">{{ $sample['actor_label'] }}</td>
                                <td class="px-5 py-3"><x-ui.badge :label="$sample['result']" semantic="success" :show-icon="false" /></td>
                                <td class="px-5 py-3"><x-ui.badge :label="$sample['severity']" semantic="notice" :show-icon="false" /></td>
                                <td class="px-5 py-3 ui-reference-text-muted">{{ $sample['route'] }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-5 py-8 text-center text-sm ui-reference-text-muted">No audit demo rows matched the current filters.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 border-t ui-reference-border px-5 py-3">
                    <div class="flex items-center gap-3">
                        <form method="GET" action="{{ route('platform.ui-reference.index') }}" class="flex items-center gap-3">
                            ...
                            <select name="audit_per_page" onchange="this.form.submit()" class="ui-select !w-auto px-3 py-2 text-sm">
                                @foreach ([10, 25, 50, 100] as $option)
                                    <option value="{{ $option }}" @selected($auditPerPage === $option)>{{ $option }}</option>
                                @endforeach
                            </select>
                        </form>
                        <p class="text-sm ui-reference-text-muted">Showing {{ $auditSamples->firstItem() ?? 0 }} to {{ $auditSamples->lastItem() ?? 0 }} of {{ $auditSamples->total() }} entries</p>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <a href="{{ $auditSamples->onFirstPage() ? '#' : $auditSamples->url($auditPrev) }}" @class([
                            ...
                            'ui-action-ghost' => ! $auditSamples->onFirstPage(),
                            'cursor-not-allowed opacity-60' => $auditSamples->onFirstPage(),
                        ])>Prev</a>
                        ...
                        <form method="GET" action="{{ route('platform.ui-reference.index') }}">
                            ...
                            <input type="hidden" name="workspace_per_page" value="{{ $workspacePerPage }}">
                            <input type="hidden" name="error_per_page" value="{{ $errorPerPage }}">
                            <select name="audit_page" onchange="this.form.submit()" class="ui-select !w-auto px-3 py-2 text-xs font-semibold uppercase tracking-[0.1em]">
                        ...
                        <a href="{{ $auditSamples->hasMorePages() ? $auditSamples->url($auditNext) : '#' }}" @class([
                            'ui-action ui-action-xs',
                            'ui-action-ghost' => $auditSamples->hasMorePages(),
                            'cursor-not-allowed opacity-60' => ! $auditSamples->hasMorePages(),
                        ])>Next</a>
                    </div>
                </div>
            </div>
