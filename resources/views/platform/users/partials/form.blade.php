<section class="flex flex-1 flex-col gap-6">
    ...
    <form method="POST" action="{{ $action }}" enctype="multipart/form-data" class="rounded-lg border border-slate-800 bg-slate-900/70 p-8">
        @csrf
        @if ($method !== 'POST')
            @method($method)
        @endif
        ...
        <div class="mt-6 space-y-6" data-staff-tab-panel="profile">
            <div class="grid gap-6 lg:grid-cols-2">
                <label class="block">
                    ...
                    <input type="text" name="first_name" value="{{ old('first_name', $user?->first_name) }}" class="mt-2 w-full rounded-md border border-slate-700 bg-slate-950 px-4 py-3 text-slate-100" required>
                </label>

                <label class="block">
                    <span class="text-sm font-semibold text-slate-200">Last Name</span>
                    <input type="text" name="last_name" value="{{ old('last_name', $user?->last_name) }}" class="mt-2 w-full rounded-md border border-slate-700 bg-slate-950 px-4 py-3 text-slate-100" required>
                </label>

                <label class="block">
                    <span class="text-sm font-semibold text-slate-200">Email</span>
                    <input type="email" name="email" value="{{ old('email', $user?->email) }}" class="mt-2 w-full rounded-md border border-slate-700 bg-slate-950 px-4 py-3 text-slate-100" required>
                </label>

                <label class="block">
                    <span class="text-sm font-semibold text-slate-200">Hourly Rate ($)</span>
                    <input type="number" min="0" step="0.01" name="hourly_rate" value="{{ old('hourly_rate', $user?->hourly_rate ?? 0) }}" class="mt-2 w-full rounded-md border border-slate-700 bg-slate-950 px-4 py-3 text-slate-100">
                </label>
                ...
                <label class="block">
                    ...
                    <input type="text" name="default_language" value="{{ old('default_language', $user?->default_language ?? config('app.locale')) }}" class="mt-2 w-full rounded-md border border-slate-700 bg-slate-950 px-4 py-3 text-slate-100">
                </label>

                <label class="block">
                    <span class="text-sm font-semibold text-slate-200">Facebook</span>
                    <input type="text" name="facebook" value="{{ old('facebook', $user?->facebook) }}" class="mt-2 w-full rounded-md border border-slate-700 bg-slate-950 px-4 py-3 text-slate-100">
                </label>

                <label class="block">
                    <span class="text-sm font-semibold text-slate-200">LinkedIn</span>
                    <input type="text" name="linkedin" value="{{ old('linkedin', $user?->linkedin) }}" class="mt-2 w-full rounded-md border border-slate-700 bg-slate-950 px-4 py-3 text-slate-100">
                </label>

                <label class="block">
                    <span class="text-sm font-semibold text-slate-200">Skype</span>
                    <input type="text" name="skype" value="{{ old('skype', $user?->skype) }}" class="mt-2 w-full rounded-md border border-slate-700 bg-slate-950 px-4 py-3 text-slate-100">
                </label>

                <label class="block">
                    <span class="text-sm font-semibold text-slate-200">Direction</span>
                    <select name="direction" class="mt-2 w-full rounded-md border border-slate-700 bg-slate-950 px-4 py-3 text-slate-100">
                ...
                <label class="block lg:col-span-2">
                    ...
                    <textarea name="email_signature" rows="3" class="mt-2 w-full rounded-md border border-slate-700 bg-slate-950 px-4 py-3 text-slate-100">{{ old('email_signature', $user?->email_signature) }}</textarea>
                </label>

                <label class="block lg:col-span-2">
                    <span class="text-sm font-semibold text-slate-200">Profile Image</span>
                    <input type="file" name="profile_image" accept="image/*" class="mt-2 w-full rounded-md border border-slate-700 bg-slate-950 px-4 py-3 text-slate-100">
                </label>

                <label class="block lg:col-span-2">
                    <span class="text-sm font-semibold text-slate-200">Password {{ $user ? '(leave blank to keep current password)' : '' }}</span>
                    <input type="password" name="password" class="mt-2 w-full rounded-md border border-slate-700 bg-slate-950 px-4 py-3 text-slate-100" @if (! $user) required @endif>
                </label>
            </div>
        ...
        <div class="mt-6 hidden space-y-6" data-staff-tab-panel="permissions">
            ...
            <div class="rounded-md border border-slate-800 bg-slate-950/70 p-5">
        ...
        <div class="mt-8 flex flex-wrap gap-3 border-t border-slate-800 pt-6">
            ...
            <a wire:navigate href="{{ route('platform.users.index') }}" class="inline-flex rounded-md border border-slate-800 px-4 py-3 text-sm font-semibold text-slate-300 transition hover:border-slate-600 hover:text-white">
                Back to Users
            </a>
        </div>
    </form>
